    ...
    ! Christiane 
    c2vis = Ini_Read_Double('c2vis',0.0d0)
    w0    = Ini_Read_Double('w0',0.0d0)
    ede   = Ini_Read_Double('ede',0.0d0)
 
    end subroutine DarkEnergy_ReadParams

    ! Christiane includes function for equation of state of variable DE 
    ! This part is copied from equations_ppf.f90 
    ! No tabulated w => use only part from if(.not. use_tabulated_w) 
    function w_de(a)
    use constants   
    real(dl) :: w_de, as, lde
    ! real(dl) :: al 
    ...
    ! Erminia's model
    ! w_de = (cs2_lam*(1.0d0+w0)/(cs2_lam-w0)*a**(-3.0d0*(1.0d0+cs2_lam))-1.0d0)/ &
    !   ((1.0d0+w0)/(cs2_lam-w0)*a**(-3.0d0*(1.0d0+cs2_lam))+1.0d0)
    
    ! Early Dark Energy 
    lde = CP%omegav

    as = 0.0000248*(1._dl+7._dl/8*(4._dl/11)**(4._dl/3)*CP%Num_Nu_massless)/(CP%H0/100)**2
    
    w_de = 1.0d0/3.0d0*((as/(1.0d0-lde))/(a+as/(1.0d0-lde))+3.0d0*w0*a**(3.0d0*w0)*(-2.0d0*a**(3.0d0*w0)*  &
       (lde-1.0d0)*ede+lde*ede+a**(6.0d0*w0)*(lde-2.0d0*ede+lde*ede))/((a**(6.0d0*w0)*(ede-1.0d0)+ede-2.0d0*   &
       a**(3.0d0*w0)*ede)*((lde-1.0d0)*ede+a**(6.0d0*w0)*(lde-1.0d0)*ede-a**(3.0d0*w0)*(lde-2.0d0*ede+2.0d0*lde*ede))))
    end function w_de  ! equation of state of the PPF DE

    ! Christiane includes new function dw/dln(a)
    function dwdlna_de(a)
    use constants  
    real(dl) :: dwdlna_de
    real(dl) :: m, n, q, r, s, lde, as
    real(dl), intent(IN) :: a

    ! Erminia
    !dwdlna_de = -3.0d0*(1.0d0+w_de(a))*(cs2_lam-w_de(a))
    ...
    m =3.0d0*w0*a**(3.0d0*w0-1.0d0)

    n =-2.0d0*a**(3.0d0*w0)*(lde-1.0d0)*ede+lde*ede+a**(6.0d0*w0)*(lde-2.0d0*ede+lde*ede)

    q = a**(6.0d0*w0)*(ede-1.0d0)+ede-2.0d0*a**(3.0d0*w0)*ede

    r =(lde-1.0d0)*ede+a**(6.0d0*w0)*(lde-1.0d0)*ede-a**(3.0d0*w0)*(lde-2.0d0*ede+2.0d0*lde*ede)

    s = a**(3.0d0*w0)*(lde-1.0d0)-lde

    dwdlna_de = 1.0d0/3.0d0*(-(as/(1.0d0-lde))/(a+as/(1.0d0-lde))**2.0d0+m*n/(q*r)- &
    3.0d0*w0*(lde-1.0d0)*m*n**2.0d0/(s*q*r**2.0d0)-3.0d0*w0*m*n**2.0d0/(r*q**2.0d0*s)- &
    m*(3.0d0*a**(3.0d0*w0)*(lde-1.0d0)*lde*ede*(1.0d0+3.0d0*w0)-lde**2.0d0*ede*(1.0d0+3.0d0*w0)+ &
    a**(9.0d0*w0)*(lde-1.0d0)*(lde-2.0d0*ede+ede*lde)*(1.0d0+3.0d0*w0)-a**(6.0d0*w0)* &
    (-6.0d0*lde*(ede+3.0d0*ede*w0)+2.0d0*(ede+6.0d0*ede*w0)+lde**2.0d0*(1.0d0-3.0d0*w0+ede*(3.0d0+9.0d0*w0))))/(s*q*r))
    end function dwdlna_de

    ! Christiane includes function for derivative of rho_de with respect to a 
    ! This part is copied from equations_ppf.f90, however with a as an input parameter instead of al 
    ! not needed here...
    function drdlna_de(a)
    real(dl) :: drdlna_de
    real(dl), intent(IN) :: a
    ! a=dexp(al)
    ...
    ! Christiane includes function for energy density of variable DE 
    ! This part is copied from equations_ppf.f90 
    ! No tabulated w => use only part from if(.not. use_tabulated_w) 
    function grho_de(a)  !8 pi G a^4 rho_de
    use constants    
    real(dl) :: grho_de
    real(dl) :: wa_ppf, w1, as, lde, omegade 
    ! real(dl) :: al, fint
    real(dl), intent(IN) :: a
    ! if(.not. use_tabulated_w) then
    wa_ppf=0.
    ...
    !grho_de=grhov*(a**4.0d0 + as/lde *ede/(1.0d0-ede))

    ! Early dark energy 
    omegade = ((lde-ede*(1.0d0-a**(-3.0d0*w0)))/(lde+(1.0d0-lde)*a**(3.0d0*w0))+ede*(1.0d0-a**(-3.0d0*w0)))
    ! vde_rho = omegade/lde*((1.0-lde-rad)*a**(-3.0)+rad*a**(-4.0))/(1.0-omegade)
    grho_de = grhov*omegade/lde*((1.0-lde-as)*a+as)/(1.0-omegade)
    ...
    !open(20,file="dvdw.dat",position="append") 
    !write(20,*) a, dwdlna_de(a)
    !close(20) 
    ...
    if (w_lam /= -1 .and. w_Perturb) then
        EV%w_ix = neq+1
        ! Christiane: increase neq by 1 in order to accommodate anisotropic stress 
        ! neq=neq+2
        ! maxeq=maxeq+2
        neq=neq+3
        maxeq=maxeq+3 
    ...
    grhov_t=grho_de(a)*a**(-2.) 
    grho=grhob_t+grhoc_t+grhor_t+grhog_t+grhov_t
    ! Christiane 
    ! gpres=(grhog_t+grhor_t)/3+grhov_t*w_lam
    gpres=(grhog_t+grhor_t)/3+grhov_t*w_de(a) 
    ...
    if (w_lam /= -1 .and. w_Perturb) then
        ...
        sigmaq=y(EV%w_ix+2) 
        ! write(*,*) sigmaq
        sigmaqdot=yprime(EV%w_ix+2)
        dgrho=dgrho + clxq*grhov_t
        ! Christiane 
        ! dgq = dgq + vq*grhov_t*(1+w_lam)
        dgq = dgq + vq*grhov_t*(1+w_de(a))
        dgpi = dgpi+ grhov_t*(1+w_de(a))*sigmaq
    ! Christiane 
    else 
        sigmaqdot=0.
    end if
    ...
    pidot_sum =  pidot_sum + grhog_t*pigdot + grhor_t*pirdot + grhov_t*(1.+w_de(a))*sigmaqdot*4./3.
    ...
    !write(*,*) 'c2vis=', c2vis  
    !open(20,file='ISW.dat',position='append')
    !write(20,*) a, ISW 
    !close(20) 
    ...
    ! Christiane 
    integer, parameter :: i_sigmaq=13 
    ! Christiane 
    ! integer, paramter :: i_max = i_vq
    integer, parameter :: i_max = i_sigmaq
    ...
    if (w_lam /= -1 .and. w_Perturb) then
        ...
        ayprime(EV%w_ix)= -3*adotoa*(cs2_de(a)-w_de(a))*(clxq+3*adotoa*(1+w_de(a))*vq/k) &
            -3.0*adotoa*a*adotoa*dwdlna_de(a)*vq/k &
            -(1+w_de(a))*k*vq -(1+w_de(a))*k*z

        ! Christiane
        ! ayprime(EV%w_ix+1) = -adotoa*(1-3*cs2_lam)*vq + k*cs2_lam*clxq/(1+w_lam)
        ayprime(EV%w_ix+1) = -adotoa*(1-3*cs2_de(a))*vq + k*cs2_de(a)*clxq/(1+w_de(a))-k*sigmaq 

        ! Christiane 
         ayprime(EV%w_ix+2) = -3*adotoa*sigmaq*(w_de(a)-1/3/adotoa*dwdlna_de(a)/(1+w_de(a)))/w_de(a) &
            +8*c2vis*(k*vq+3*ayprime(2)/k+k*z)/3/(1+w_de(a))
        ! write(*,*) ayprime(EV%w_ix+2) 
    ...
    if (EV%TightCoupling) then
        ...
        gpres=gpres+ (grhog_t+grhor_t)/3 +grhov_t*w_de(a)
    ...
    gpres=(grhog_t+grhor_t)/3._dl+grhov_t*w_de(a) 
